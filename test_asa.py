...
sim = SimulationASA(fr, c, size=0.16, ds=ds, device=device)
print(sim.dim)


sim.add_transducer(
    Transducer(16, 0.16, ds, [-0.1, 0, 0], Orientation.X, t_mux=t_mux, device=device)
)
sim.add_transducer(
    Transducer(16, 0.16, ds, [-0.1, 0, 0], Orientation.Y, t_mux=t_mux, device=device)
)
sim.add_transducer(
    Transducer(16, 0.16, ds, [-0.1, 0, 0], Orientation.Z, t_mux=t_mux, device=device)
)
...
sample = torch.roll(create_donut_rot(sim_dim, 5, 32, [45, 45, 0]), 0, 0)
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 600, gamma=0.8)
losses = []
