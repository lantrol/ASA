...
class SinCosModel(nn.Module):
    ...
    def forward(self, x, training=False) -> torch.Tensor:
        # x shape: [B, 1, 64, 64] or [B, 64, 64]
        if x.dim() == 3:
            x = x.unsqueeze(1)
        ...
        # Transformer (treat the latent as a sequence of length 1)
        ...
        if training:
            ...
            return phases, reconstruction, mu, logvar
    ...
    def get_phases(self, x) -> torch.Tensor:
        ...
        # We can't use the training path directly because we don't want reconstruction/mu/logvar
        # but we need the same forward logic. We'll call forward with training=False.
        # However, forward(training=False) returns only phases.
        # Let's ensure the return type is consistent for the user.

        # To avoid code duplication, let's use the logic from forward:
        # Since forward(training=False) returns just phases, we can use it.
