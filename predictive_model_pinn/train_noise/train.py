...
if __name__ == "__main__":
    ...
    lr = 0.0001
    criterions = [CosineLoss, ScaleInvariantMSELoss, NCCLoss]
    ...
    for crit in criterions:
        ...
        plt.grid(True)
        plt.savefig(
            f"{Path(__file__).resolve().parents[0]}/{criterion.__class__.__name__}.png"
        )
        plt.show()

        # Save the model
        model_path = f"trained_model_rand_{criterion.__class__.__name__}.pth"
        ...
        if LOG_TRAIN:
            ...
            with open(
                f"{Path(__file__).resolve().parents[0]}/{criterion.__class__.__name__}.json",
                "w",
            ) as json_file:
