...
class SimulationASA:
    ...
    def create_propagators(self):
        for transducer in self.transducers:
            propagator = torch.zeros(
                (self.dim, self.dim, self.dim), dtype=torch.complex64
            )

            # Create the coordinate grid
            z_coords = (
                torch.arange(-self.dim // 2, self.dim // 2).view(1, 1, -1).float()
                * self.ds
                + self.pos[0]
            )
            y_coords = (
    ...
    def calculate_volume(self):
        ...
        volume = torch.zeros((self.dim, self.dim, self.dim), dtype=torch.complex64)
...
class Transducer:
    def __init__(
        ...
        self.phases = torch.zeros(
            (emitters_num, emitters_num), dtype=torch.float32, requires_grad=True
        )
    ...
    def to_complex_plane(self, ds: float):
        ...
        gap_between = (
            target_size - cells_per_emitter * self.phases.size()[0]
        ) // self.phases.size()[0]
